...
bool MessagingChannel::CreateMapping()
{
  ...
  if (!(mappingObject = CreateFileMappingFromApp(INVALID_HANDLE_VALUE, NULL, PAGE_READWRITE, size, this->mapping.GetName().data())))
  {
    return false; // Failed to create mapping object
  }

  if (!(this->mapping.GetAddress() = MapViewOfFileFromApp(mappingObject, FILE_MAP_ALL_ACCESS, 0, 0)))
  {
